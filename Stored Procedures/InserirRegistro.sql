...
--Inserir uma verificação se a conexao e o hidrometro estão ativas.
	...
	IF (SELECT COUNT(ID) FROM REGRA WHERE HIDROMETROFK = HIDROMETROCOLETA AND ATIVO = 1) > 0 THEN --EXISTEM REGRAS ATIVAS
		...
		VALORINICIOPERIODO := (SELECT MIN(ID) FROM REGISTRO WHERE DATA BETWEEN DATAPERIODO AND (DATAPERIODO + REGRAPERIODO));
		VALORFIMPERIODO := (SELECT MAX(ID) FROM REGISTRO WHERE DATA BETWEEN DATAPERIODO AND (DATAPERIODO + REGRAPERIODO));
