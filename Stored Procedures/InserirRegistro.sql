...
BEGIN
	IF ((SELECT ATIVO FROM HIDROMETRO WHERE ID = HIDROMETROCOLETA) = 1) THEN
		...
		IF (SELECT COUNT(ID) FROM REGRA WHERE HIDROMETROFK = HIDROMETROCOLETA AND ATIVO = 1) > 0 THEN --EXISTEM REGRAS ATIVAS
			REGRAPERIODO := (SELECT PERIODO FROM REGRA WHERE HIDROMETROFK = HIDROMETROCOLETA LIMIT 1);
			REGRAVALOR := (SELECT VALOR FROM REGRA WHERE HIDROMETROFK = HIDROMETROCOLETA LIMIT 1);
			REGRAID := (SELECT ID FROM REGRA WHERE HIDROMETROFK = HIDROMETROCOLETA LIMIT 1);
			DATAPERIODO := (SELECT dt_inicio_periodo FROM REGRA WHERE HIDROMETROFK = HIDROMETROCOLETA LIMIT 1);
