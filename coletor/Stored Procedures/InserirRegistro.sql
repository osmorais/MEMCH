	...
	IF (SELECT REGRATIPOFK FROM REGRA WHERE HIDROMETROFK = hidrometrocoleta) <> 3 THEN --REGRA DE CONSUMO OU DE SURTO
		...
		IF(VALORPERIODO > REGRAVALOR) THEN
			IF(SELECT REGRATIPOFK FROM REGRA WHERE HIDROMETROFK = hidrometrocoleta) = 2 THEN -- REGRA DE CONSUMO
				DESCRICAOREGRA := CONCAT('ALERTA DE CONSUMO - O registro do dia ',datacoleta,' ultrapassou o limite de coleta em ',(coleta)-REGRAVALOR,'.');
				INSERT INTO ALERTA (DESCRICAO, DATA, HIDROMETROFK, REGRAFK)
				VALUES (DESCRICAOREGRA,datacoleta,hidrometrocoleta,REGRAID);
			ELSE
				DESCRICAOREGRA := CONCAT('ALERTA DE SURTO - O registro do dia ',datacoleta,' apresentou um consumo fora do comum estipulado.');
				INSERT INTO ALERTA (DESCRICAO, DATA, HIDROMETROFK, REGRAFK)
				VALUES (DESCRICAOREGRA,datacoleta,hidrometrocoleta,REGRAID);
			END IF;
		END IF;
